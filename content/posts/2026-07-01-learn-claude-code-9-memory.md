---
title: Learn Claude Code -9-Memory
slug: 2026-07-01-learn-claude-code-9-memory
description: '"压缩会丢细节, 要有一层不丢的" — 文件仓库 + 索引 + 按需加载，跨压缩、跨会话。 Harness 层 : 记忆 —
  跨压缩、跨会话的知识积累。 问题 s8 的 autoCompact 会把当前目标、剩余工作、用户约束写进摘要，但细节会丢失："用 tab
  缩进不要用空格"可能被简化成"用户有代码风格偏好"。而且…'
author: 墨韵
date: 2026-07-01
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 396df5c0-26f4-80d7-adde-f8816018323c
notionSyncedAt: 2026-07-10T13:55:04.503Z
---

> _"压缩会丢细节, 要有一层不丢的"_ — 文件仓库 + 索引 + 按需加载，跨压缩、跨会话。
>
...
s8 的 autoCompact 会把当前目标、剩余工作、用户约束写进摘要，但细节会丢失："用 tab 缩进不要用空格"可能被简化成"用户有代码风格偏好"。而且新开一个会话，连摘要也没了。
...
![memory-overview.svg](images/blog/2026-07-01-learn-claude-code-9-memory/img-591b3a319b.svg)


s8 的压缩管线保留，聚焦记忆。存储选文件系统：`.memory/` 目录下，每个记忆一个 `.md` 文件，带 YAML frontmatter（`name` / `description` / `type`）。文件多了需要索引：`MEMORY.md` 一行一个链接，注入 SYSTEM。
...
![memory-subsystems.svg](images/blog/2026-07-01-learn-claude-code-9-memory/img-289f1f811c.svg)
...
CC 把这个过程叫 Dream，实际有四层门控：时间间隔、扫描节流、会话数、文件锁。本文简化为文件数阈值。
...
| 文件                                            | 行数  | 职责                                                                                                                       |
| --------------------------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------ |
| `memdir/memdir.ts`                            | 507 | 核心：[MEMORY.md](http://memory.md/) 定义（`34-38`）、记忆行为指令区分 memory/plan/tasks（`199-266`）、`loadMemoryPrompt()` 三条路径（`419-490`） |
...
| `memdir/memoryScan.ts`                        | —   | 扫描 .md 文件，排除 [MEMORY.md](http://memory.md/)，读 frontmatter，最多 200 个，按 mtime 降序（`35-94`）                                   |
...
1. `memoryScan.ts` 扫描 `.memory/` 下所有 `.md` 文件（排除 [MEMORY.md](http://memory.md/)），最多 200 个，按 mtime 降序
...
触发位置（`stopHooks.ts:141-155`）：在 `handleStopHooks()` 中，fire-and-forget 触发提取和 Dream。本文把提取放在 `stop_reason != "tool_use"` 分支里，方向一致。
...
CC 用 Markdown + YAML frontmatter，和本文一致。四种类型：`user`、`feedback`、`project`、`reference`。
...
- **Team memory**：团队共享记忆，`loadMemoryPrompt()` 有专门路径
