---
title: Learn Claude Code -15- Agent Teams
slug: 2026-07-04-learn-claude-code-15-agent-teams
description: '"一个搞不定, 组队来" — 文件收件箱 + 队友线程。 Harness 层 : 团队 — 多 Agent 协作, 消息总线。 问题
  "重构整个后端"涉及认证模块、数据库层、API 路由、测试。一个 Agent 在修 API
  路由时，认证模块的细节已经不在上下文里了。上下文窗口就那么大，单个 Agent 的注意力覆盖不…'
author: 墨韵
date: 2026-07-04
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-80d3-8f7f-f89dc8b22a7d
notionSyncedAt: 2026-07-10T13:54:23.559Z
---

> _"一个搞不定, 组队来"_ — 文件收件箱 + 队友线程。
>
...
s6 的子 Agent 是临时工，叫来干一件事就走了。但有些任务需要能通信、能协作的队友。
...
![agent-teams-overview.svg](images/blog/2026-07-04-learn-claude-code-15-agent-teams/img-5b6c90fbc6.svg)


沿用 S14 的能力（prompt 组装、任务系统、后台执行、cron 调度）。为了聚焦团队机制，省略了完整错误恢复、记忆和技能系统。新增三样：**MessageBus**（文件收件箱）、**spawn_teammate_thread**（启动队友线程）、**inbox 注入**（Lead 接收队友消息并注入 history）。
...
|      | s6 子 Agent            | s15 队友                         |
| ---- | --------------------- | ------------------------------ |
| 生命周期 | 一次性，用完销毁              | 多轮（本文限 10 轮，真实 CC 用 idle loop） |
| 通信   | 只回传结论                 | 异步收件箱，随时通信                     |
| 上下文  | 完全隔离                  | 通过消息共享信息                       |
...
![team-topology.svg](images/blog/2026-07-04-learn-claude-code-15-agent-teams/img-f88aa11524.svg)
...
为什么用文件而不是内存队列？本文选文件是因为直观、跨线程可观察。真实 CC 也用文件收件箱（`~/.claude/teams/{team}/inboxes/`），但加了 `proper-lockfile` 防并发写冲突。本文的 `read_inbox` 有 read + unlink 竞态，多线程同时读可能丢消息。
...
- **队友有简化工具集**：bash、read、write、send_message。本文省略了任务和 cron，聚焦通信机制。真实 CC 的队友也有 TaskCreate、TaskUpdate 等工具，任务系统是团队共享的
- **本文限 10 轮**：防止队友无限循环。真实 CC 用 idle loop：跑完一轮后发 `idle_notification`，等 inbox 消息，收到后继续，直到 `shutdown_request` 才退出
...
本文在用户输入循环外注入。CC 更精细，Lead 的 `useInboxPoller` 每 1 秒检查一次，有消息就提交为新的 turn，不需要等用户输入。


### 权限冒泡


本文省略了权限冒泡。真实 CC 的流程（`permissionSync.ts`、`useSwarmPermissionPoller.ts`）：
...
本文用 `MessageBus` 类收发消息。CC 的做法更直接，每个 Agent 直接写其他 Agent 的收件箱文件。
...
本文省略了权限冒泡。CC 的实际流程（`permissionSync.ts`）：
