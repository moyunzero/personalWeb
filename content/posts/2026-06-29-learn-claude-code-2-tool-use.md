---
title: Learn Claude Code -2-Tool Use
slug: 2026-06-29-learn-claude-code-2-tool-use
description: '"加一个工具, 只加一个 handler" — 循环不用动, 新工具注册进 dispatch map 就行。 Harness 层 :
  工具分发 — 扩展模型能触达的边界。 只有 bash 一个工具 Agent 只有一个 bash 工具。读文件要 ，写文件要 ，改文件要 。
  模型想的是"读这个文件"，却要拼出 。多了一层…'
author: 墨韵
date: 2026-06-29
categories:
  - note
tags:
  ...
  - LLM
draft: false
notionId: 396df5c0-26f4-8028-95ef-e7e3de8f3aad
notionSyncedAt: 2026-07-10T13:56:11.314Z
---
...
Agent 只有一个 bash 工具。读文件要 `cat`，写文件要 `echo "..." > file.py`，改文件要 `sed`。
...
![tool-dispatch.svg](images/blog/2026-06-29-learn-claude-code-2-tool-use/img-bdac19e5df.svg)


s1 的循环完全保留（LLM 调用、stop_reason 判断、消息追加）。唯一的变动在工具执行那 1 行：`run_bash()` 替换为 `TOOL_HANDLERS[block.name]()` 查表分发。
...
模型经常一次返回多个 tool_use："读一下 [a.py](http://a.py/) 和 [b.py](http://b.py/)，然后列出所有 .py 文件"。


按 `response.content` 原始顺序逐个执行。CC 的做法更复杂：按原始顺序切成连续 batch，batch 内并发安全的工具并行执行，batch 间严格顺序（见附录）。
...
| 多工具调用         | 模型可一次返回多个 tool_use，本文按原始顺序逐个执行 |
...
| 循环   | `while True` + `stop_reason` | 与 s1 完全一致                    |
...
文章：`TOOLS` 数组 + `TOOL_HANDLERS` 字典。定义和实现分开。
...
![%E6%88%AA%E5%B1%8F2026-07-07_12.22.19.png](images/blog/2026-06-29-learn-claude-code-2-tool-use/img-eb5868117e.png)
...
1. **Zod schema 验证**（`614-680`）：参数类型/结构检查
2. **工具级 validateInput()**（`682-733`）：参数值验证（如路径是否在工作区内）
3. **PreToolUse hooks**（`800-862`）：钩子可以返回消息、修改输入、阻止执行
4. **权限检查**（`921-931`）：canUseTool + checkPermissions → allow/deny/ask
5. **执行 tool.call()**（`1207-1222`）

### 五、流式工具执行


CC 的 `StreamingToolExecutor`（`StreamingToolExecutor.ts`）让工具在模型还在生成时就启动——不等模型说完。`read_file` 可能在模型还在输出"我来分析"的时候就跑完了。
