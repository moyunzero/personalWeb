---
title: Learn Claude Code -4-Hooks
slug: 2026-06-30-learn-claude-code-4-hooks
description: '"挂在循环上, 不写进循环里" — hook 在工具执行前后注入扩展逻辑。 Harness 层 : hook — 扩展点不侵入循环。
  问题 s3 的 Agent 有权限检查了。但每次加一个新检查，比如"记录每次 bash 调用"、"操作后自动 git add"，都要修改 函数。
  循环很快就变成了这样： 你想扩展的是 A…'
author: 墨韵
date: 2026-06-30
categories:
...
s3 的 Agent 有权限检查了。但每次加一个新检查，比如"记录每次 bash 调用"、"操作后自动 git add"，都要修改 `agent_loop` 函数。
...
PreToolUse 的非 None 返回值会阻止本次工具执行，Stop 的非 None 返回值会强制续跑。UserPromptSubmit 和 PostToolUse 的返回值未被使用。


**UserPromptSubmit**，用户输入提交后、进入 LLM 前触发。CC 中可以拦截或修改输入：
...
**PreToolUse / PostToolUse**，工具执行前后的 hook。s3 的权限检查逻辑现在包装成 PreToolUse hook，再加一个日志 hook 和一个大输出提醒：


```python
# PreToolUse: 权限检查（s3 的逻辑，从循环移到 hook）
...
**Stop**，循环即将退出时触发（`stop_reason != "tool_use"`）。用于打印收尾统计：
...
**循环里只改了一处**：s3 直接调用 `check_permission(block)`，s4 改为 `trigger_hooks("PreToolUse", block)`：
...
for block in response.content:
    ...
    # s3: if not check_permission(block): ...
    # s4: hook 替代硬编码
...
上文只讲了 PreToolUse 和 PostToolUse。CC 实际有 27 个 hook 事件（`coreTypes.ts:25-53`）：
...
4 个核心事件（UserPromptSubmit、PreToolUse、PostToolUse、Stop），因为它们覆盖了一个完整 agent cycle 的关键节点。其他 23 个都是同样的模式。
