---
title: "Learn Claude Code -10-System Prompt "
slug: 2026-07-02-learn-claude-code-10-system-prompt
description: '"prompt 是组装出来的, 不是写死的" — 分段 + 按需拼接 + 缓存。 Harness 层 : 提示 — 运行时组装,
  不硬编码。 问题 从 s1 到 s9，system prompt 都是一行硬编码： s1 够用，只有 bash、read、write 三个工具。但到
  s9，Agent 已经有记忆、有压缩、有…'
author: 墨韵
date: 2026-07-02
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 396df5c0-26f4-8007-bbf8-ff722b8c2250
notionSyncedAt: 2026-07-10T13:54:49.815Z
---

> _"prompt 是组装出来的, 不是写死的"_ — 分段 + 按需拼接 + 缓存。
>
...
从 s1 到 s9，system prompt 都是一行硬编码：
...
s1 够用，只有 bash、read、write 三个工具。但到 s9，Agent 已经有记忆、有压缩、有技能加载。prompt 该提的能力越来越多：
...
![system-prompt-overview.svg](images/blog/2026-07-02-learn-claude-code-10-system-prompt/img-4683a84a69.svg)


核心变动：把硬编码的 `SYSTEM` 拆成独立段落（section），运行时根据真实状态按需拼接，缓存结果避免重复组装。
...
| ---- | ---------------------- | --------------------------------------------- |
| 内容   | gitStatus、cacheBreaker | [CLAUDE.md](http://claude.md/) 内容、currentDate |
...
标准交互模式下 system prompt 核心约 20-30KB 文本。CLAUDE_CODE_SIMPLE 约 150 字符。用户上下文（[CLAUDE.md](http://claude.md/)）和系统上下文（git status）在此基础上累加。
