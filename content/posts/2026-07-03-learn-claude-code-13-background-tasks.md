---
title: Learn Claude Code -13-Background Tasks
slug: 2026-07-03-learn-claude-code-13-background-tasks
description: '"慢操作丢后台, agent 继续处理" — 后台线程跑命令, 完成后注入通知。 Harness 层 : 后台 — 异步执行,
  不阻塞主循环。 问题 你用过洗衣机吗？把衣服扔进去，按下启动，然后去干别的——做饭、回消息、看论文。30
  分钟后洗衣机"滴滴滴"提醒你：好了。你不会站在洗衣机前面干等 30 分钟。 Agent…'
author: 墨韵
date: 2026-07-03
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-80e6-b73c-daa513cc4403
notionSyncedAt: 2026-07-10T13:54:32.577Z
---

> _"慢操作丢后台, agent 继续处理"_ — 后台线程跑命令, 完成后注入通知。
>
...
![background-tasks-overview.svg](images/blog/2026-07-03-learn-claude-code-13-background-tasks/img-ca662f6885.svg)


沿用 S12 的简化任务系统和 prompt 组装；为了聚焦后台任务，省略完整错误恢复、记忆和技能系统。唯一的变动：慢操作扔到后台线程，Agent 继续跑循环，后台完成后把通知注入到对话里。
...
模型通过 bash 工具的 `run_in_background` 参数显式请求后台执行。如果模型没指定，本文用关键词启发式兜底：
...
CC 的 bash 工具 schema 里有 `run_in_background: boolean` 参数（`BashTool.tsx:241`）。模型自己决定哪些命令丢后台，不靠关键词猜。本文保留启发式作为兜底，但主路径是模型显式请求。
...
返回 `bg_id` 而不是只返回 `[Running in background...]`。`daemon=True` 确保 Agent 进程退出时线程跟着退出。本文用内存字典追踪状态；真实 CC 有 `LocalShellTaskState`，输出重定向到文件，支持停止任务、读取后续输出等完整生命周期。
...
本文在 agent loop 继续运行时轮询后台结果。真实 CC 通过通知队列（`messageQueueManager.ts`）把后台完成事件送入后续 turn，不需要等工具循环。
