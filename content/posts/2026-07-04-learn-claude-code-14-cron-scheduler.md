---
title: Learn Claude Code -14-Cron Scheduler
slug: 2026-07-04-learn-claude-code-14-cron-scheduler
description: '"按时间表生产工作, 调度与执行解耦" — cron 调度, 持久化或会话级。 Harness 层 : 调度 — 独立线程判断时间,
  队列传递触发。 问题 闹钟不需要你盯着它才会响。你设好 7:00，到点它自己响，你在睡觉、在洗澡、在做饭，它都照响不误。 s13 让 Agent
  能后台执行慢操作，但所有操作仍然是你手动…'
author: 墨韵
date: 2026-07-04
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-8096-8d3a-d5dce209f71b
notionSyncedAt: 2026-07-10T13:54:27.875Z
---

> _"按时间表生产工作, 调度与执行解耦"_ — cron 调度, 持久化或会话级。
>
...
![cron-scheduler-overview.svg](images/blog/2026-07-04-learn-claude-code-14-cron-scheduler/img-37e7d1b3e8.svg)


代码沿用 S13 的简化任务系统、后台执行和 prompt 组装；为了聚焦调度器，省略完整错误恢复、记忆和技能系统。新增：独立的 cron 调度线程，每秒检查一次，时间到了把任务塞进 `cron_queue`；再由 queue processor 在 Agent 空闲时自动交付。
...
本文实现的是最小 queue processor：用 `agent_lock` 判断 Agent 是否空闲，空闲时自动交付定时任务。真实 CC 的 `useQueueProcessor.ts` 还会处理 UI 阻塞、队列优先级和不同消息模式。
...
真实 CC 通过 `useQueueProcessor.ts:48-60` 在无 query、无阻塞 UI、队列非空时自动触发处理。`queueProcessor.ts:52-87` 按队列优先级把命令交给 `handlePromptSubmit()`。本文用 `queue_processor_loop` 保留核心行为：队列有任务且 Agent 空闲时，自动启动一轮 agent_loop。
