---
title: Learn Claude Code -12-Task System 
slug: 2026-07-03-learn-claude-code-12-task-system
description: '"大目标拆成小任务, 排好序, 持久化" — 文件持久化的任务图, 多 agent 协作的基础。 Harness 层 : 任务 —
  持久化的目标, 可恢复的进度。 问题 Agent 接到一个项目：搭数据库、写 API、加测试。它用 s5 的 TodoWrite 列了一张清单，然后开始写
  API，写到一半发现没数据库表，…'
author: 墨韵
date: 2026-07-03
categories:
...
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-80ec-be18-fc15d56a5f9a
notionSyncedAt: 2026-07-10T13:54:38.232Z
---

> _"大目标拆成小任务, 排好序, 持久化"_ — 文件持久化的任务图, 多 agent 协作的基础。
>
...
Agent 接到一个项目：搭数据库、写 API、加测试。它用 s5 的 TodoWrite 列了一张清单，然后开始写 API，写到一半发现没数据库表，回头补；加测试时发现 API 接口签名又变了...


盖房子不能先盖屋顶再打地基。任务之间有先后。任务依赖应该形成有向无环图（DAG）；本文只演示 `blockedBy` 检查，没有实现环检测。


s5 的 TodoWrite 是当前任务的执行清单，保存在会话内存中。这里需要的是**任务系统**：每个任务是一个 JSON 文件，任务之间有 `blockedBy` 依赖，跨会话持久化在磁盘上。
...
![task-system-overview.svg](images/blog/2026-07-03-learn-claude-code-12-task-system/img-883ee562d5.svg)


保留基础 agent loop，为聚焦任务系统省略了 S11 的完整错误恢复（RecoveryState、退避、升级、reactive compact、fallback model）。新增 5 个任务工具 + `.tasks/` 目录持久化 + `blockedBy` 依赖检查。任务系统与错误恢复是独立层：CC 源码中 `utils/tasks.ts` 只管 CRUD，`query.ts` 的 with_retry/RecoveryState 管错误恢复，互不耦合。


TodoWrite vs Task System：


|      | TodoWrite (s5)                    | Task System (s12)                 |
| ---- | --------------------------------- | --------------------------------- |
...
![task-dag.svg](images/blog/2026-07-03-learn-claude-code-12-task-system/img-14126c9eee.svg)
...
CC 没有 `in_progress → pending` 的 release 路径。如果 teammate 终止或 shutdown，CC 会把它未完成的任务 unassign（清除 owner），并将 status 重置为 `pending`，方便其他 agent 重新认领。本文省略了这一恢复路径。
...
本文只讲了 id、subject、status、owner、blockedBy。CC 实际有 9 个字段（`utils/tasks.ts:76-89`）：
...
注意：本文把 claim 和开始工作合成一步（claim = set owner + in_progress）；真实 CC 的 `claimTask` 主要解决 owner 竞争，只设 owner 不改 status，状态更新由 `TaskUpdate` 完成。
...
CC 的任务系统有四个工具（不是本文的一个通用 Task 工具）：`TaskCreate`、`TaskGet`、`TaskUpdate`、`TaskList`。全部设置 `isConcurrencySafe: true` 和 `shouldDefer: true`（工具 schema 不在初始 prompt 中，需 ToolSearch 后才可见）。


本文的 `create_task(blockedBy=...)` 在创建时直接声明依赖，是合理简化。真实 CC 的 `TaskCreate` 只接受 subject/description/activeForm/metadata，依赖关系由 `TaskUpdate` 的 `addBlocks/addBlockedBy` 维护。
