---
title: Learn Claude Code -8-Context Compact
slug: 2026-07-01-learn-claude-code-8-context-compact
description: '"上下文总会满, 要有办法腾地方" — 四层压缩策略, 便宜的先跑贵的后跑。 Harness 层 : 压缩 — 干净的记忆,
  无限的会话。 问题 Agent 跑着跑着，不动了。 手里有 bash、有 read、有 write，能力是够的。但它读了一个 1000 行的文件（ 4000
  token），又读了 30 个文件，…'
author: 墨韵
date: 2026-07-01
categories:
...
tags:
  ...
  - Agent
draft: false
notionId: 396df5c0-26f4-8012-b385-f06f05226efb
notionSyncedAt: 2026-07-10T13:55:20.157Z
---

> _"上下文总会满, 要有办法腾地方"_ — 四层压缩策略, 便宜的先跑贵的后跑。
>
...
![compact-overview.svg](images/blog/2026-07-01-learn-claude-code-8-context-compact/img-70fe6e9edf.svg)


核心变动：每轮 LLM 调用前插入三层预处理器（0 API），token 仍超阈值时触发 LLM 摘要（1 API），API 报错时应急裁剪。
...
![compaction-layers.svg](images/blog/2026-07-01-learn-claude-code-8-context-compact/img-f8308b27ba.svg)


### L1: snip_compact — 裁掉无关的旧对话


Agent 跑了 80 轮对话，`messages` 攒了 160 条。最前面的"帮我创建 [hello.py](http://hello.py/)"和当前工作几乎无关了，但全占着位置。
...
![micro-compact.svg](images/blog/2026-07-01-learn-claude-code-8-context-compact/img-ad6f1a7763.svg)
...
![layer1-budget.svg](images/blog/2026-07-01-learn-claude-code-8-context-compact/img-e2741ee534.svg)
...
![auto-compact.svg](images/blog/2026-07-01-learn-claude-code-8-context-compact/img-f70ed5a684.svg)
...
3. **替换消息列表**：所有旧消息被替换为一条摘要。本文只保留摘要；真实 Claude Code 会在 compact 后重新附加部分最近文件、计划、agent/skill/tool 等上下文。
...
本文为了讲解方便按 L1/L2/L3/L4 编号，但实际执行顺序和编号不完全对应：


| 维度                 | 本文                           | Claude Code                                                                                                                                                                     |
| ------------------ | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 执行顺序               | budget → snip → micro → auto | budget → snip → micro → collapse → auto（`query.ts:379-468`）                                                                                                                     |
| snip_compact       | 保留头 3 + 尾 47                 | CC 仅主线程启用；实现不在开源仓库中（`HISTORY_SNIP` feature gate），但接口可见：`snipCompactIfNeeded(messages)` → `{ messages, tokensFreed, boundaryMessage? }`，还暴露了 `SnipTool` 工具让模型主动调用。本文的 3/47 是简化参数 |
...
| 后压缩恢复              | 无（本文只保留摘要）                   | 自动重新读取最近文件、计划、agent/skill/tool 等                                                                                                                                                |
...
4. `contextCollapse`（L441）：独立的上下文管理系统（本文无）
5. `autoCompact`（L454）：LLM 全量摘要

本文的 budget → snip → micro 顺序与此一致。本文没有 contextCollapse 机制。


### read_file 的取舍


本文的 `micro_compact` 会把旧 `tool_result` 统一替换成占位符，包括 `read_file`。这通常不影响功能正确性：如果后续还需要文件内容，模型可以重新读一次。代价是可能多一次工具调用，也可能降低 prompt cache 命中率。


Claude Code 没有用本文这种简单规则解决这个问题。它把 `Read` 也放进可 microcompact 的工具集合，但同时维护 `readFileState`：重复读取未变化文件时返回 `FILE_UNCHANGED_STUB`，compact 后再按预算恢复最近读过的文件内容（例如最多 5 个文件、每个 5K token、总预算 50K token）。这是生产级实现里的缓存和恢复机制，本文不展开，保留“压缩旧结果，必要时重新读取”的简单 trade-off。
...
- **sessionMemoryCompact**：compact_history 之前，CC 会先尝试用已有的 session memory做轻量摘要，不调 LLM。
