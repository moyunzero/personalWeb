---
title: Learn Claude Code -3-Permission
slug: 2026-06-29-learn-claude-code-3-permission
description: '"工具执行前先做权限判断" — 权限管线决定哪些操作需要审批。 Harness 层 : 权限 — 在工具执行前加一道门。 问题 s2
  的 Agent 有 5 个工具。file tools 受 保护，但 bash 不受限制。让它"清理一下项目"，可能执行 。
  安全不能靠信任模型，要靠代码——在工具执行之前做判断。 解决方…'
author: 墨韵
date: 2026-06-29
categories:
  - note
...
s2 的 Agent 有 5 个工具。file tools 受 `safe_path` 保护，但 bash 不受限制。让它"清理一下项目"，可能执行 `rm -rf /`。
...
在工具执行前插入 `check_permission()`——每个工具调用经过三道闸门，顺序固定：硬拒绝优先，软询问次之，都没命中就放行。
...
三道闸门（deny → ask → allow）和 CC 不完全对应。CC 的 `PermissionResult` 有 4 个 behavior（`types/permissions.ts:241-266`）：
