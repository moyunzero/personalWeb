---
title: Learn Claude Code -17-Autonomous Agents
slug: 2026-07-04-learn-claude-code-17-autonomous-agents
description: '"自己看板，自己认领" — 空闲时轮询，有活就干。 Harness 层 : 自治 — 队友自组织，不依赖 Lead 分配。 问题
  s16 的队友能通信、能握手关机。但每个队友等 Lead 分配任务——如果任务看板上有 10 个未认领任务，Lead 得手动 assign 10
  次。这不能扩展。队友应该自己看任务看板，发现…'
author: 墨韵
date: 2026-07-04
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-8085-8b79-f35fe04b9e2e
notionSyncedAt: 2026-07-10T13:54:13.052Z
---

> _"自己看板，自己认领"_ — 空闲时轮询，有活就干。
>
...
![autonomous-agents-overview.svg](images/blog/2026-07-04-learn-claude-code-17-autonomous-agents/img-8c90d3ac95.svg)


沿用 S16 的本文 MessageBus 和协议工具。新增：**idle_poll**（空闲时每 5 秒轮询一次）、**scan_unclaimed_tasks**（扫描看板上可认领的任务）、**自动认领**（找到任务就 claim，不用 Lead 操心）。
...
三个条件：必须是 pending、没有 owner、所有 blockedBy 依赖已完成。`can_start` 检查依赖任务的状态——有依赖不代表不能做，只有被未完成的任务阻塞才不能做。本文按文件名排序取第一个；CC 用文件锁防止多个队友同时认领同一个任务。
...
本文没有文件锁，并发认领可能出现竞争。但至少 `task.owner` 检查避免了最明显的"后写覆盖"问题。CC 用 `proper-lockfile` 保护任务文件，`claimTask` 在文件锁内完成读-改-写（`utils/tasks.ts:541-612`）。
...
autoCompact（s8）之后，队友的 messages 列表可能被压缩成一段摘要。每次进入新的 WORK 阶段时检查：
...
消息过短说明发生了压缩，此时重新注入身份信息。真实 CC 中 context compaction 会保留 system prompt，本文的简化实现需要手动处理。
...
CC 的实际实现是多个机制的组合，但目标一致——减少 Lead 的手动分配负担。


### 一、CC 的空闲机制：组合路径，不是单一轮询


本文用一个 `idle_poll()` 统一处理空闲时的 inbox 检查和任务认领。CC 的实际实现是四个机制的组合：
...
本文的 `idle_poll()` 把 CC 的四个机制合并成一个轮询函数——简化合理，因为核心语义（空闲时找活干、依赖解锁后可认领、shutdown 优先）是一致的。
