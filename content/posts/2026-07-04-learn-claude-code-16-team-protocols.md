---
title: Learn Claude Code -16- Team Protocols
slug: 2026-07-04-learn-claude-code-16-team-protocols
description: '"队友之间要有约定" — request response 模式驱动协商。 Harness 层 : 协议 — Agent
  之间的结构化握手。 问题 s15 的队友能干活了，但协调是松散的：Lead 发消息，队友回复，没有结构化的协议。两个场景暴露了问题： 关机 ：Lead 想让
  Alice 关机。直接杀线程，Alice…'
author: 墨韵
date: 2026-07-04
categories:
  - note
tags:
  ...
  - Agent
draft: false
notionId: 399df5c0-26f4-8011-93f8-e5f2feb3d511
notionSyncedAt: 2026-07-10T13:54:17.941Z
---

> _"队友之间要有约定"_ — request-response 模式驱动协商。
>
...
![team-protocols-overview.svg](images/blog/2026-07-04-learn-claude-code-16-team-protocols/img-0d27939ac1.svg)


在 S15 团队通信基础上加入结构化协议。为了聚焦协议机制，省略了完整错误恢复、记忆和技能系统。新增三样：**ProtocolState**（请求状态追踪）、**dispatch_message**（按消息类型路由到处理器）、**match_response**（通过 request_id 关联回复与请求，含类型校验）。
...
> 本文演示了计划审批的请求-响应消息流程，没有实现执行门控（未 approved 时拦截 bash/write_file）。真实 CC 的队友有 permission gating 机制。
...
> 本文用 `shutdown_response` 统一命名（approve 字段区分同意/拒绝）。真实源码拆成 `shutdown_approved` 和 `shutdown_rejected` 两种独立消息类型（`teammateMailbox.ts:720-763`）。
...
本文省略了 idle_notification 给 Lead 的通知。真实 CC 在 idle 时发 `idle_notification`，Lead 收到后知道队友空闲，可以分配新任务。
...
CC 的团队协议实现（`teammateMailbox.ts`，1184 行）和本文在核心结构上一致：request_id + approve/reject 的请求-响应模式。差异在于：


**关机协议**：CC 的 shutdown 是三向通信（`teammateMailbox.ts:720-763`、`SendMessageTool.ts:268-430`）。Lead 发 `shutdown_request`，队友回复 `shutdown_approved`（或 `shutdown_rejected` 附原因），系统发送 `teammate_terminated` 通知所有相关方。关机确认后系统自动清理 pane（tmux/iTerm2）、unassign 任务、从 team config 移除成员（`useInboxPoller.ts:677-800`）。本文用 `shutdown_response` 统一命名，真实源码拆成 approved/rejected 两种独立消息。
...
**消息格式**：CC 的协议消息是结构化的 JSON（有 Zod schema 验证），本文用简单的 type + metadata 字典。字段名也不统一：permission 用 `request_id`（`teammateMailbox.ts:453-462`），shutdown 和 plan approval 用 `requestId`（`teammateMailbox.ts:684-763`）。


**执行门控**：CC 的队友有完整的 permission gating。未获批准的高风险操作会被拦截，不是可选的。本文只演示了消息流程，没有实现执行拦截。


**通用性**：本文的一个 FSM（pending → approved | rejected）对应两种协议，这个简化完全正确。CC 的所有协议消息共用同一个 request id 关联机制。
